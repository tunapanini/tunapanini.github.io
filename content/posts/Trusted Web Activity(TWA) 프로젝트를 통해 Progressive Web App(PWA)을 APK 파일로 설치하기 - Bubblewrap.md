...
description: "Bubblewrap CLI로 PWA를 Android APK로 변환하는 과정 — 프로젝트 생성부터 DigitalAssetLinks 설정까지"
date: 2020-05-26
tags:
  - typescript
  - react
...
draft: false
---
Trusted Web Activity(TWA)는 Custom Tabs를 기반으로 한 프로토콜을 사용하여 Progressive Web App(PWA) 또는 다른 웹앱 컨텐츠를 열 수 있는 새로운 방법입니다.

배포 상태의 PWA를 Android 프로젝트를 만들어 APK 파일로 설치하고 싶고, 단순하게 앱을 열면 PWA가 실행되는 기능(`Add to Home Screen`과 동일한 기능)만 원한다면,

`Bubblewrap` Node.js 라이브러리를 이용하여 TWA 프로젝트를 쉽게 생성하고 빌드하여 APK 파일을 만들 수 있습니다.
...
맨 처음 init 커맨드를 실행할 때, `JDK`와 `Android SDK` Path를 입력하게 됩니다. 각각 설정된 Path를 수동으로 입력하면 됩니다.
...
그리고 TWA 정보를 입력합니다. `manifest`에서 받아온 값을 기반으로 하기 때문에 문제가 없다면 기본값을 그대로 사용하면 됩니다.

![](https://blog.kakaocdn.net/dn/P3HDw/btqEqsj2oa4/fb8uh6Szgo7WHllHJn9PT0/img.png)

마지막에 `Location of the Signing Key`와 `Key name`은 기존에 생성한 키가 있다면 별도로 설정해주면 됩니다.

프로젝트 생성이 끝난 후 `Signing Key`가 없는 경우 자동으로 새로 생성할 것인지 물어봅니다. 질문에 따라 정보를 입력하면 지정한 이름으로 `.keystore` 파일이 생성됩니다.
...
`bubblewrap install` 커맨드를 이용하면 adb를 사용하여 바로 실행할 수도 있습니다.

# DigitalAssetLinks 추가

APK를 실행하면 웹브라우저처럼 URL 입력창이 활성화된 화면이 표시됩니다. 앱처럼 보이게 하려면 [DigitalAssetLinks](https://developers.google.com/digital-asset-links/v1/getting-started?hl=ko)를 추가해야 합니다.

아래 방법 중 하나로 JSON 파일을 작성하여 PWA가 있는 도메인의 `.well-known/assetlinks.json`으로 접근할 수 있도록 추가합니다.
...
위 앱을 설치하기 번거롭다면 직접 작성합니다. `package_name`과 `앱 서명 인증서의 SHA256 지문`만 찾아서 예시와 같이 작성하면 됩니다.
...
SHA256 지문은 아래 커맨드를 실행했을 때 나오는 값 중에서 찾아 사용합니다.

```
keytool -list -v -keystore android.keystore
```

DigitalAssetLinks를 추가하고 상단바가 없어진 모습입니다.
